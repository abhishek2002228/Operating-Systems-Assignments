        ...
        for(int j = 0; j < s; j++){
            if(board[i][j] == 0){
				...
				for(int k = 0; k < reduceSpace.n; k++){
                	...
                	if(pc%10000){
                	pc = 0;
                	printf("\r backtracking with r = %d , c =%d , v = %d "  , i,j,c);
                	fflush(stdout);
                	}
                            board[i][j] = 0; 
                            }
...
void *depth4Solver(void *val){
    ...
    int r = -1,c = -1;
        ...
        while(k!=0 && r!=-1 && c!=-1 && opts.n==1){
		    ...
		    printf("Fill in thread 4 %d on row: %d, col: %d\n",opts.avl_options[0],r,c);
...
void *depth3Solver(void *val){
    ...
    if(check(l->board,l->row,l->col,l->val_attempt,l->size) == 1){
        ...
        if(k != 0 && r != -1 && c != -1){
            ...
            pthread_t forthLayer[36];
            game *gb4 = (game*) malloc(opts.n * sizeof(game)); 
            for(int i = 0; i < opts.n; i++){
                memcpy(gb4[i].board, l->board, sizeof (int) *36* 36);
                gb4[i].size = l->size;
                gb4[i].t_id = i;
                gb4[i].val_attempt = opts.avl_options[i];
                gb4[i].row = r;
                gb4[i].col = c;
                tc_glob++;
                if(tc_glob > NUM_THREADS){
                    printf("Ideal thread count exceeded \n");
                }
                if(pthread_create(&forthLayer[i],NULL,depth4Solver, (void *)&gb4[i])){
            ...
            for(int i = 0; i < opts.n; i++){
                pthread_join(forthLayer[i],NULL);
                if(gb4[i].solved){
